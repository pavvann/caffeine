...
/**
 * Write a {@link Pipeline} back to `<repoPath>/pipeline.md`,
 * preserving the markdown body that follows the closing `---` fence.
 * If the file does not exist, a stub body is included.
 *
 * The serializer is intentionally narrow — same shape the parser
 * accepts, no flow style, no exotic scalars. Stage names with
 * non-`[A-Za-z0-9_-]` characters or commands containing newlines are
 * rejected so the round-trip stays lossless.
 */
export async function writePipeline(
  repoPath: string,
  pipeline: Pipeline,
): Promise<void> {
  validateForWrite(pipeline);
  const path = join(repoPath, "pipeline.md");
  const frontmatter = serializeFrontmatter(pipeline);
  ...
  try {
    ...
    body = "\n# Pipeline\n\nGenerated by the Caffeine pipeline editor.\n";
  }
...
function validateForWrite(p: Pipeline): void {
  for (const name of p.per_task) {
    if (!/^[A-Za-z0-9_-]+$/.test(name)) {
      throw new PipelineParseError(
        `invalid stage name for write: ${JSON.stringify(name)} (allowed: letters, digits, underscore, hyphen)`,
      );
    }
  }
  for (const step of p.on_backlog_complete) {
    if (typeof step.run !== "string" || step.run.includes("\n")) {
      throw new PipelineParseError(
        `invalid on_backlog_complete entry: run must be a single-line string`,
      );
    }
  }
  if (
    !Number.isInteger(p.decider.max_iterations) ||
    p.decider.max_iterations < 1
  ) {
    throw new PipelineParseError(
      "decider.max_iterations must be a positive integer for write",
    );
  }
}

function serializeFrontmatter(p: Pipeline): string {
  const lines: string[] = [];
  lines.push("per_task:");
  for (const stage of p.per_task) lines.push(`  - ${stage}`);
  lines.push("on_backlog_complete:");
  for (const step of p.on_backlog_complete) lines.push(`  - run: ${step.run}`);
  lines.push("decider:");
  lines.push(`  max_iterations: ${p.decider.max_iterations}`);
  if (p.decider.cost_ceiling_per_iteration_usd !== undefined) {
    lines.push(
      `  cost_ceiling_per_iteration_usd: ${p.decider.cost_ceiling_per_iteration_usd}`,
    );
  }
  return lines.join("\n") + "\n";
}
...
/**
 * Return everything after the frontmatter's closing fence in `text`.
 * If the file has no frontmatter, returns the entire text — caller
 * decides what to do with it (writePipeline replaces frontmatter and
 * keeps body, so a fenceless input becomes the body of a fresh write).
 */
